...
%2    
    ...
    subplot(211),     plot(freq_cap, abs(fft(IRleft_cap)), 'b');
    %No = 4.79945e6; dN =  1400;
    %V=axis;
    %axis([No/Fs (No+dN)/Fs V(3) V(4)]);                  
    ...
    %
    %subplot(212),     plot(Sweepright, 'r'); hold on;
    subplot(212),     plot(freq_cap, abs(fft(Sweepright_cap)), 'r'); hold on;
                      %plot(abs(fftSweepright), 'b');
    %V=axis; axis([No/Fs (No+dN)/Fs -1 1]);                  
    %V=axis; axis([4.79945e6 4.8009e6 V(3) V(4)]);                  
