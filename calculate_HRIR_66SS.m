%% calculate_HRIR_xx.m
% %
% Analyze recordings, compress them using the AmpESS sweep signal.
...
%
%
%...66          SS  A = S is completed. Now shift A to really capture A.
...
%% CALCULATE SPECTROGRAM
    ...
    %[Ssig_ts ,timets ,freqts ] = myspectrogram_01(ts ,Fs,nfft,norm,dB,dBmin,dBmax);
    %[Ssig_rec,timerec,freqrec] = myspectrogram_01(sig_left,Fs,nfft,norm,dB,dBmin,dBmax);
    %[Ssig_shr,timeshr,freqshr] = myspectrogram_01(shorted_left,Fs,nfft,norm,dB,dBmin,dBmax);
...
%% CALCULATE AUTO- AND CROSS-CORRELATION
    ...
    [shortedcor,shortedfcor,shortedcortime,shortedcorfreq,N] = myxcorr_03test(shorted_left,ts',Fs,fdom,dB,dBmin,dBmax);
    [croscor,fcor,cortime,corfreq,N] = myxcorr_03(sig_left,ts',Fs,fdom,dB,dBmin,dBmax);
...
%2  PLOT CORRELATION    
    figure(2), hold off
        subplot(211), 
    plot(tcor, ShortedS, 'r-'), hold on;
    plot(tcor, ShortedA, 'b-')
    axis('tight'); grid on;
    title('Shorted Audio (Setup: PC Scarlett')
        subplot(212), 
    plot(tcor, S, 'r-'); hold on;
    plot(tcor, A, 'b-');
    axis('tight'); grid on;
    title('RoomIR (Setup:Kirk head - Loudspeaker - PC)')
    ...
    fSmax   = max(abs(fS));   %Now use this factor also to normalize A!
    fS      = fS ./ fSmax;
    fA      = fA ./ fSmax;
    ...
    freq = [1:length(fshortS)]./(length(fshortS)).*Fs;
    
    figure(3), clf
        subplot(311),
    plot(freq, 20 .* log10(abs(fshortS)), 'r-'); hold on;
    plot(freq, 20 .* log10(abs(fshortA)), 'b-');
    V=axis; axis([0 Fs/2 -70 10]); grid on;
    title('Transfer function (Cable transmission)');
        subplot(312),
    plot(freq, 20 .* log10(abs(fS)), 'r-'); hold on;
    plot(freq, 20 .* log10(abs(fA)), 'b-');
    V=axis; axis([0 Fs/2 -70 10]); grid on;
    title('Transfer function (Short distance loudspeaker transmission)');
        subplot(313),
    plot(freq, 20 .* log10(abs(fSnorm)), 'r-'); hold on;
    plot(freq, 20 .* log10(abs(fAnorm)), 'b-');
    V=axis; axis([0 Fs/2 -70 10]); grid on;
    title('Divide Transfer function / TFofthecable');
    ...
    plot(freq, phase_shortA,'b-'); grid on;
    V=axis; axis([0 Fs/2 V(3) V(4)]); grid on;
    title('Transfer function (Cable transmission)');
    
    subplot(312), 
    plot(freq, phase_S,'r-'); hold on; 
    plot(freq, phase_A,'b-'); grid on;
    V=axis; axis([0 Fs/2 V(3) V(4)]); grid on;
    title('Transfer function (Short distance loudspeaker transmission)');        
    
    subplot(313), 
    plot(freq, phase_Snorm,'r-'); hold on;
    plot(freq, phase_Anorm,'b-'); grid on;
    V=axis; axis([0 Fs/2 V(3) V(4)]); grid on;
    title('Divide Transfer function / TFofthecable');
