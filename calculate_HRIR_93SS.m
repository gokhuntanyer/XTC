...
%% CALCULATE AUTO- AND CROSS-CORRELATION
    ...
    d1 = 38;  %this is to shift S to align with A.
    CENS([1: 2*Ndel+1]) = cntrcor   ([N_S-Ndel+NN2+d1 : N_S+Ndel+NN2+d1]);
    CENA([1: 2*Ndel+1]) = cntrcor   ([N_A-Ndel+NN2 : N_A+Ndel+NN2]);
    
    SIGS([1: 2*Ndel+1]) = sigLcor   ([N_S-Ndel+NN2+d1 : N_S+Ndel+NN2+d1]);
    SIGA([1: 2*Ndel+1]) = sigLcor   ([N_A-Ndel+NN2 : N_A+Ndel+NN2]);
    
    twin = zeros(1,4097);
    twin([1:100]) = ones(1,100);
    twin([101:150]) = [49:-1:0]./49;
    ...
    %CENS = CENS - sum(CENS); %this is DC filter just to show TF.
    %CENA = CENA - sum(CENA);
    %SHRS = SHRS - sum(SHRS);
    %SHRA = SHRA - sum(SHRA);
    %SIGS = SIGS - sum(SIGS);
...
%2  PLOT CORRELATION    
    ...
    %set(gca,'xtick',[Fmn  100 1000 10000 Fmx]); grid on;
    legend('HRTF  S/A Center','location','Southwest');
    xlabel('Frequency (Hertz)')
    ylabel('Desibels');
    ...
    %V=axis; axis([Fmn Fmx  -80 20]); grid on;
    %set(gca,'xtick',[Fmn  100 1000 10000 Fmx]); grid on;
    legend('HRTF Normalized S/A','location','Southwest');
    %title ('Divide TF of loudspeaker / TF of cable');
    xlabel('Frequency (Hertz)')
    ylabel('Desibels');
