...
%
%
%...09          OK  XTC-MONO-L is done. Now right.
%...08          SS  OK. Loads STEREO and saves to XTC-STEREO. Now do mono.
...
%%  Load music file, save to MONO, then play on ONE CHANNEL with XTC
    filename = 'Cuban55-48K.wav';
    %filename = 'CubanSalsa.mp3';
    [dum, FS] = audioread(filename);
    %Duration = 5;  %duration of music
    %Ntime = Duration .* FS;
    %dum2([1:Ntime],:) = dum([1:Ntime],:);
    ...
    %L([1:Ntime],1) = dum([1:Ntime],1);
    %R([1:Ntime],1) = dum([1:Ntime],2);
    len = length(R);
    close all; clear dum;
...
%%  Now we have all the filters;  S, A, invS, invA, 1/(S+A) and 1/(S-A)  
    ...
    filename = 'CubanSalsa-XTC-Stereo.wav';
    audiowrite(filename,REC1,Fs);
