...
%
%
%             18SS  This is the version I am using once again.
%...18  180412  ..  Test waveform.
%This part is after I discover that S and A were all WRONG!
...
%%  CREATE THE TRANSMITTER SIGNAL   / also the SHORTED SIGNAL
    ...
    sLEF  = [s2 pL ]; %this is really the LEFT tested it on my headphones!
    sRIG  = [s2 pR ];
    sSHR  = [s2 pSH];
    %sLEF = [s2 pL s2 pL];
    %sRIG = [s2 pR s2 pR];
  ...
  dataSH = [slong sSHR, sSHR,  sSHR, sSHR,  sSHR, sSHR,  sSHR, sSHR,  sSHR, sSHR, ...
    ...
    figure(3), clf
    subplot(211), 
    plot(timed, data(1,:), 'r-'), hold on, axis('tight');
    title('Left Channel')
    
    subplot(212), 
    plot(timed, data(2,:), 'b-'), hold on
    %plot(timed, data(2,:), 'k+'), 
    axis('tight');
