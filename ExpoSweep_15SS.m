...
%
%
%                       I had to use the conj to have correct looking
%                       results. Need to study why I have a different
%                       formula.
%....15             OK  Deconvolution using ifft(fft(y)./conj(fft(x)))
...
fprintf('\n\nRunning:______________\n');
    ...
    Hfilter = fft(stfilter);
    Hfilter_phase = unwrap(atan2(imag(Hfilter), real(Hfilter)));
    
    Yvoice = fft(Voice);
    Yvoice_phase = unwrap(atan2(imag(Yvoice), real(Yvoice)));
    
    Himpres = (Yvoice) ./ conj(Hfilter);
    Himpres_phase = unwrap(atan2(imag(Himpres), real(Himpres)));
    ...
    subplot(411),
    plot(freq,abs(Hfilter))
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    title('abs(Hfilter)');
    
    subplot(412),
    plot(freq,Hfilter_phase)
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    title('phase(Hfilter)');
    
    subplot(413),
    plot(freq,abs(Yvoice))
    title(Yvoice);
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    title('abs(fft(Y))');
    
    subplot(414),
    plot(freq,Yvoice_phase)
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    title('phase(fft(Y))');
    
    figure(2),clf, hold off
    subplot(211),
    plot(freq,abs(Himpres))
    title('abs(fft(h(t))');
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    
    subplot(212),
    plot(freq,Himpres_phase)
    axis('tight');V=axis;axis([0 Fs/2 V(3) V(4)]);
    title('Phase distribution of fft(h(t)');
    
    figure(3),clf, hold off
    plot(time-0.1259, ImpRes)
    axis('tight');V=axis;axis([0 0.03 V(3) V(4)]);
    title('Time windowed/shifted h(t)');
...
fprintf('Reading file\n');
    ...
    dum1 = zeros(size(dum(:,1))); 
    dum1(:,1) = dum(:,1); 
    
    Nlast = 1146000;%length(dum1);
    Nfrst = Nlast - length(stfilter)+1;   %270000;
    
    Voice = zeros(size(stfilter)); %length(stfilter),1);
    Voice(1, [1:(Nlast-Nshft)]) = dum1([Nshft:Nlast-1],1)';
    clear dum;clear dum1;
    Voice = Voice - sum(Voice)./length(Voice);  %filter out the DC component
fprintf('IR HT (Cross-correlation):\n');
    
    
    %save('stfilter', 'stfilter');
    %save('Voice', 'Voice');
    ...
    dum = conv(stfilter',Voice,'full');
    dumtime = toc/60;  %runtime in minutes
fprintf('\n Cross-correlation calculated at the %4.0f minutes.\n', dumtime);
    lendum = length(dum);
    Npeak = 882194%1152193; %keep data in {Npeak-57, Npeak+1437}
    %XCorr = dum;
    XCorr([1: 57+1437+1],1) = dum([Npeak-57:Npeak+1437],1);
